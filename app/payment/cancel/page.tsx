...
const CancelPaymentPage = () => {
  return (
    <div className="bg-background relative flex min-h-screen flex-col items-center justify-center overflow-hidden px-4 py-16">
      {/* Animated background gradient - Red theme */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -top-1/2 -left-1/2 size-[800px] animate-pulse rounded-full bg-red-500/10 blur-3xl" />
        <div
          className="absolute -right-1/2 -bottom-1/2 size-[800px] animate-pulse rounded-full bg-red-500/5 blur-3xl"
      ...
      <div className="fade-in-up relative z-10 flex flex-col items-center space-y-8 text-center">
        ...
        <div className="relative">
          ...
          >
            <XCircle
              className="size-32 text-red-500 md:size-40"
              strokeWidth={1.5}
            />
          </div>
          {/* Glowing effect */}
          <div className="absolute inset-0 -z-10 animate-pulse">
            <div className="absolute top-1/2 left-1/2 h-full w-full -translate-x-1/2 -translate-y-1/2 bg-red-500/20 blur-3xl" />
          </div>
        </div>
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        {[...Array(12)].map((_, i) => (
          <div
            key={i}
            className="animate-float absolute h-1 w-1 rounded-full bg-red-500/30"
            style={{
              left: `${10 + i * 8}%`,
              top: `${20 + (i % 5) * 15}%`,
